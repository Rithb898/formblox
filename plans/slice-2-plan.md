...
- [ ] Extend `forms.list` (or a light count query) to include a per-form response count for the dashboard cards. *(Not done — skipped as optional. Responses page shows its own count; form cards link to responses via the dropdown.)*
...
- [ ] Closed form / unpublished → submit blocked server-side. *(Code mirrors `getBySlug`; not explicitly tested.)*
- [ ] Rate-limit triggers after threshold. *(Helper wired + per-IP key; not load-tested.)*
- [ ] `git commit`, deploy — could submit right now. *(No commit made yet.)*
